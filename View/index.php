...
<body>
    ...
    <div class="container"> 
        <div class="sidebarDoar">
            <h1>Doar</h1>
            <p class="voluntarios">
                Se você veio até aqui porque pode contribuir com algum item, fique tranquilo, so precisaremos de pouquissimas informações. Clique aqui para fazer este cadastro rapido!
            </p>
            <div class="btn_ajudar">
            ...
            <div class="overlay_doacao" style="display:<?php echo isset($_GET['minhas_doacao']) ? 'block' : 'none'; ?>">
                ...
                <div class="cadastradas">
                    <form id="form_escolhida_<?=$minha['codigo_doacao_minha']?>"action="../Controller/cancelarDoacaoController.php"method="POST"> 
                        ...
                        <span><?=$vencimento ?></span></h2>

                        <h2 class="opcoes"><span><?= !empty($minha['telefone_transporte_minha']) ? 'Um entregador Já aceitou fazer este transporte! ' : 'Nenhum entregador aceitou ainda' ?></span></h2> 
                        <h2 class="opcoes">Nome de quem vai transportar: <br><span><?= $minha['nome_transporte_minha']?></span></h2>
        ...
        <div class="content">
            <h1 class="titulo">Acionando a Vida</h1>
            <p class="motivo">
        ...
        <div class="sidebarTransportar">
            <h1>Transportar</h1>
            <p class="voluntarios">
                Se você pode ajudar ao proximo disponibilizando seu tempo e recursos para fazer o transporte de algum item doado. Cique aqui e cadastre apenas informações basicas suas, e vera listado os items disponiveis para trasnporte.
            </p>
                ...
                <?php
                    if(isset($_SESSION['doacoes']) && !empty( $_SESSION['doacoes'])){
                        ...
                        foreach ($_SESSION['doacoes'] as $doacao):
                            ...
                            $data_atual = new DateTime();
                            $vencimento = ($data_doacao < $data_atual) ? 'Data Vencida, entre em contato com o Doador para saber se ainda está disponivel!':$doacao['data_doacao'];
                ?>
            ...
            <div class="overlay_escolher"style="display:<?php echo isset($_GET['escolhidas_doacao']) ? 'block' : 'none'; ?>">
                ...
                <?php
                    if(isset($_SESSION['escolhidas']) && !empty( $_SESSION['escolhidas'])){
                        $nome = $_SESSION['nome_transporte'];
                        echo "<h1 style = padding-top:5%;>$nome, Você tem ".count($_SESSION['escolhidas'])." doações a entregar!</h1>";
                        foreach ($_SESSION['escolhidas'] as $meus):
                        $data_doacao = DateTime::createFromFormat('d/m/Y H:i',$meus['data_doacao_escolhida']);
                        $data_atual = new DateTime();
                        $vencimento = ($data_doacao < $data_atual) ? 'Data Vencida, entre em contato com o Doador para saber se ainda está disponivel!':$meus['data_doacao_escolhida'];    
                ?>
